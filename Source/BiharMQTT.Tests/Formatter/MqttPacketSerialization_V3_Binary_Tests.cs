...
public sealed class MqttPacketSerialization_V3_Binary_Tests
{
    ...
    public void DeserializeV311_MqttConnectPacketWithWillMessage()
    {
        var p = new MqttConnectPacket
        {
            ...
            WillMessage = "Good byte."u8.ToArray(),
    ...
    public void DeserializeV311_MqttPublishPacket()
    {
        var p = new MqttPublishPacket
        {
            ...
            Dup = true,
            Retain = true,
            PayloadSegment = new ArraySegment<byte>("HELLO"u8.ToArray()),
    ...
    public void Serialize_LargePacket()
    {
        ...
        publishPacket.PayloadSegment = new ArraySegment<byte>("MQTT"u8.ToArray());
    ...
    static MqttProtocolVersion DeserializeAndDetectVersion(MqttPacketFormatterAdapter packetFormatterAdapter, byte[] buffer)
    {
    ...
    static byte[] Serialize(MqttPacket packet, MqttProtocolVersion protocolVersion)
    {
        return MqttPacketFormatterAdapter.GetMqttPacketFormatter(protocolVersion, WriterFactory()).Encode(packet).Join().ToArray();
    }

    static void SerializeAndCompare(MqttPacket packet, string expectedBase64Value, MqttProtocolVersion protocolVersion = MqttProtocolVersion.V311)
    {
        Assert.AreEqual(expectedBase64Value, Convert.ToBase64String(Serialize(packet, protocolVersion)));
    }

    static MqttBufferWriter WriterFactory()
    {
