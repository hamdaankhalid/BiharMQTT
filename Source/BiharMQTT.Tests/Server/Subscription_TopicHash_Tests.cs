...
public sealed class SubscriptionTopicHashTests : IDisposable
{
    ...
    public void Check_Hash_Bucket_Depth()
    {
        ...
        TopicGeneratorHelper.Generate(numPublishers, numTopicsPerPublisher, out var topicsByPublisher, out _, out _);
    ...
    async Task<List<string>> PrepareTopicHashSubscriptions(TopicHashSelector selector)
    {
        ...
        TopicGeneratorHelper.Generate(numPublishers, numTopicsPerPublisher, out var topicsByPublisher, out var singleWildcardTopicsByPublisher, out var multiWildcardTopicsByPublisher);
    ...
    static class TopicGeneratorHelper
    {
            ...
            out Dictionary<string, List<string>> multiWildcardTopicsByPublisher)
        {
            ...
            if (topicsPerLevel <= 0) topicsPerLevel = 1;
            ...
            if (maxNumLevel3Topics <= 0) maxNumLevel3Topics = 1;
            ...
            for (var p = 0; p < numPublishers; ++p)
            {
                ...
                for (var l1 = 0; l1 < numLevel1Topics; ++l1)
                {
                    for (var l2 = 0; l2 < numLevel2Topics; ++l2)
                    {
                        ...
                        {
                            if (publisherTopicCount >= numTopicsPerPublisher) break;

                            var topic = $"{publisherName}/building{l1 + 1}/level{l2 + 1}/sensor{l3 + 1}";
                            AddTopic(publisherName, topic, topicsByPublisher);

                            if (l2 == 0)
                            {
                                var swt = $"{publisherName}/building{l1 + 1}/+/sensor{l3 + 1}";
                                AddTopic(publisherName, swt, singleWildcardTopicsByPublisher);
                            }

                            if (l1 == 0 && l3 == 0)
                            {
                                var mwt = $"{publisherName}/+/level{l2 + 1}/+";
                                AddTopic(publisherName, mwt, multiWildcardTopicsByPublisher);
                            }
        ...
        static void AddTopic(string publisher, string topic, Dictionary<string, List<string>> dict)
        {
            if (!dict.TryGetValue(publisher, out var list))
            {
                list = new List<string>();
                dict.Add(publisher, list);
            }
            list.Add(topic);
        }
    }
