...
namespace BiharMQTT.Tests.Extensions;
...
public sealed class MqttTopicTemplate_Tests
{
    ...
    public void CanonicalPrefixFilter()
    {
        ...
        var canonicalFilter = MqttTopicTemplate.FindCanonicalPrefix([template1, template2, template3]);
        ...
        canonicalFilter = MqttTopicTemplate.FindCanonicalPrefix([template1, template2, template3, template2B]);
        ...
        var canonicalFilter2 = MqttTopicTemplate.FindCanonicalPrefix([template1, template2, template3, template4]);
    ...
    {
        var template = MqttTopicTemplate.FindCanonicalPrefix(
        [
    ...
    public void DynamicRoutingSupport()
    {
        ...
        var incomingV1Topic = "A/v1/32/F";

        var compatV2Topic = v2Template.WithParameterValuesFrom(v1Template.ParseParameterValues(incomingV1Topic)).WithParameter("bar", "unknown");

        // id: 32 was relocated and bar: unknown added
        Assert.AreEqual("A/v2/foo/32/unknown", compatV2Topic.Template);
    }
    ...
    public void SendAndSubscribeSupport()
    {
        ...
        var filter = new MqttTopicFilterBuilder().WithTopicTemplate(template).WithAtLeastOnceQoS().WithNoLocal().Build();

        Assert.AreEqual("App/v1/+/message", filter.Topic);

        var myTopic = template.WithParameter("sender", "me, myself & i");

        var message = new MqttApplicationMessageBuilder().WithTopicTemplate(myTopic).WithPayload("Hello!").Build();
    ...
    public void SubscriptionSupport()
    {
        ...
        var filter = template.BuildFilter().WithAtLeastOnceQoS().WithNoLocal().Build();
    ...
    public void SubscriptionSupport2()
    {
        ...
        var subscribeOptions = new MqttClientFactory().CreateSubscribeOptionsBuilder().WithTopicTemplate(template).WithSubscriptionIdentifier(5).Build();
